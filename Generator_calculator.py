...
def output_number_abbreviation(number_out: int):
    ...
    for suffix, exponent in abbreviation_table.items():
        if (((output_length - 1) // 3) * 3) == exponent:
            if len(divided_number) >= 6:
                divided_number = divided_number[:5]
                if int(divided_number[4]) > 0:
                    divided_number = divided_number[:-1] + str(int(divided_number[-1]) + 1) #+1 for round up, -1 for round down
            
            return divided_number[:5] + suffix     
